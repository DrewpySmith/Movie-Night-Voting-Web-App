<x-guest-layout>
    <div class="grid min-h-[calc(100vh-72px)] place-items-center px-5 py-12">
        <div class="w-full max-w-md animate-rise rounded-[2rem] border border-white/10 bg-surface p-8 shadow-2xl shadow-black/30">
            ...
            <h1 class="mt-6 text-center text-3xl font-black text-white">Welcome back</h1>
            <p class="mt-2 text-center text-zinc-400">Sign in to manage your rooms and votes.</p>
            ...
            <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-4">
                ...
                <div class="flex items-center justify-between">
                    <label for="remember_me" class="flex items-center gap-2 text-sm text-zinc-400">
                        <input id="remember_me" type="checkbox" name="remember" class="rounded border-white/10 bg-surface-elevated text-red-500 focus:ring-red-500">
                        Remember me
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm text-zinc-400 underline transition hover:text-white">
                            Forgot password?
                        </a>
                    @endif
                </div>
                ...
                <p class="text-center text-sm text-zinc-500">
                    ...
                    <a href="{{ route('register') }}" class="text-red-400 font-bold hover:text-red-300">Sign up</a>
                </p>
            </form>
        </div>
    </div>
